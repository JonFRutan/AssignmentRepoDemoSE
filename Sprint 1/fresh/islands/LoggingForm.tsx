...
export default function LoggingForm() {
  ...
  const handleSubmit = (event) => {
    ...
    // Validation
    if (log.pain_level === 0) errors.push("Please select a pain level.");
    if (log.quality_of_life === 0) errors.push("Please select a quality of life level.");
    if (log.satisfaction === 0) errors.push("Please select a satisfaction level.");
    if (log.social_quality === 0) errors.push("Please select a social quality level.");
    if (weight !== 0 && weight < 50) errors.push("Invalid weight: must be between 50-1000 lbs");
    ...
    const normalizedDate = new Date(date).toISOString(); // Ensure ISO format

    if (normalizedDate) {
      const storedCompletedDays = JSON.parse(localStorage.getItem("completedDays") || "{}");
      storedCompletedDays[normalizedDate] = {
        isCompleted: true,
        formData: log,
      };
      ...
      console.log("Logging form submission:");
      console.log("Normalized date:", normalizedDate);
      console.log("Updated completedDays:", JSON.stringify(storedCompletedDays, null, 2));
    }
